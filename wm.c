...
int main(void) {
	...
	while (1) {
		...
		case XCB_KEY_PRESS: {
			...
			xcb_configure_window(conn, key_event->child, mask, &value);

			xcb_set_input_focus(conn, XCB_INPUT_FOCUS_PARENT, 
				key_event->child, XCB_CURRENT_TIME);
		}
		break;
		// begin move/resize
		case XCB_BUTTON_PRESS: {
			...
			xcb_get_geometry_reply_t* reply = xcb_get_geometry_reply(conn, 
				xcb_get_geometry(conn, wnd), NULL);
		...
		case XCB_MOTION_NOTIFY: {
			...
			xcb_configure_window(conn, wnd, mask, &values);
		}
		break;
		}
